...
const validateInput = (input) => isNaN(input) || input < 0 || input > guessingRange - 1
...
function displayRemainingAttempts(totalAttempts, remainingAttempts) {
    console.clear();
    console.log(chalk.bgYellow(` Remaining Attempts: ${Array.from({ length: totalAttempts })
        .fill("❤️", 0, remainingAttempts)
        .fill("🩶", remainingAttempts)
        .join(" ")}  \t\t\t`), chalk.bgMagentaBright(` Your Score: ${newScore} \n`));
}
function promptNumberInput() {
    ...
    prompts.next({
        ...
        message: chalk.blueBright(`Guess a number between 0 and ${guessingRange - 1}:`),
...
function promptDifficulty() {
    ...
    prompts.next({
        ...
        default: guessingRange === 10 ? "easy" : guessingRange === 100 ? "medium" : "hard",
    });
}
    ...
    switch (questionName) {
        case "mainmenu":
            switch (answer) {
                case "newgame":
                    ...
                    displayRemainingAttempts(totalNumberOfAttempts, totalNumberOfAttempts);
                    newScore = 0;
                    promptNumberInput();
                    break;
                case "difficulty":
        ...
        case "guess":
            ...
            if (guess === randomNumber) {
                ...
                randomNumber = generateNumber.next().value;
                const newAttempt = attempts !== undefined
